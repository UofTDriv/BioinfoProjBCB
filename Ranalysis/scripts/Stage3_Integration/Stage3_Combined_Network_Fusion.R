...
parse_args <- function() {
  ...
  list(
    ...
    species_abundance = kv$species_abundance %||%
      here("data", "processed", "species_list_unaligned.csv"),
    annotated_species = kv$annotated_species %||%
      here("outputs", "annotated_species260330.csv"),
...
load_species_matrix <- function(path) {
  ...
  if (length(sample_cols) == 0) {
    ...
    if (length(alt) > 0) {
      warning("No sample-level cladeReads columns found in species file. Falling back to: ", alt[[1]])
  ...
  if (length(sample_cols) == 0) {
    stop("Species abundance file must contain sample columns ending with '_cladeReads'.")
  }

  mat <- sp %>%
    select(name, all_of(sample_cols)) %>%
    rename_with(~ str_replace(.x, "_cladeReads$", ""), all_of(sample_cols))
...
cat("[Stage3] Building HOMD ecological niche network...\n")
