...
parse_args <- function() {
  ...
  list(
    filtered_gene_counts = kv$filtered_gene_counts %||%
      here("data", "processed", "h3n2_aligned_filtered_gene_counts.csv"),
    species_abundance = kv$species_abundance %||%
      here("outputs", "reportRanalysisH3N2", "outputs", "nonhuman_bracken260330op.csv"),
    species_list_nonhuman = kv$species_list_nonhuman %||%
      here("outputs", "reportRanalysisH3N2", "outputs", "species_list_nonhuman260330op.csv"),
    annotated_species = kv$annotated_species %||%
      here("outputs", "reportRanalysisH3N2", "outputs", "annotated_species260330.csv"),
    deseq_dir = kv$deseq_dir %||% here("outputs", "h3n2_DESeq2_results_bracken"),

    padj_threshold = as.numeric(kv$padj_threshold %||% 0.5),
    log2fc_threshold = as.numeric(kv$log2fc_threshold %||% 0.05),
    species_corr_threshold = as.numeric(kv$species_corr_threshold %||% 0.03),
    gene_corr_threshold = as.numeric(kv$gene_corr_threshold %||% 0.06),
    ...
    out_graphml = kv$out_graphml %||%
      here("outputs", "UNFILT_h3n2_Stage3_Network_for_RISK_bracken.graphml"),
    out_homd_graphml = kv$out_homd_graphml %||%
      here("outputs", "UNFILT_h3n2_Stage3_Network_HOMD_Niche_bracken.graphml"),
    out_annotation_csv = kv$out_annotation_csv %||%
      here("outputs", "UNFILT_h3n2_Stage3_RISK_Annotations_bracken.csv"),
    out_validation_csv = kv$out_validation_csv %||%
      here("outputs", "UNFILT_h3n2_Stage3_Jaccard_Validation_bracken.csv"),
    out_fgsea_csv = kv$out_fgsea_csv %||%
      here("outputs", "UNFILT_h3n2_Stage3_fgsea_go_bp_results_bracken.csv"),
    out_species_layer = kv$out_species_layer %||%
      here("outputs", "UNFILT_h3n2_Stage3_Species_only_bracken.graphml"),
    out_gene_layer = kv$out_gene_layer %||%
      here("outputs", "UNFILT_h3n2_Stage3_Gene_only_bracken.graphml")
  )
}
...
load_species_matrix <- function(path, species_list_path = NULL) {
  ...
  if (length(sample_cols) == 0) {
    fallbacks <- c(
      here("outputs", "reportRanalysisH3N2", "outputs", "nonhuman_bracken260330op.csv"),
      here("outputs", "reportRanalysisH3N2", "outputs", "nonhuman_merged.csv"),
      here("outputs", "reportRanalysisH3N2", "outputs", "unaligned_merged.csv")
    )
